...
__global__ void indexed_sparse_gemv_bf16(
    ...
    __nv_bfloat16* __restrict__ output,           // [batch, M]
    int M,
  ...
  for (int base = 0; base < number_non_zeros; base += topKChunkSize) { //within each split-k, can load topkchunksize 
    int const count = min(topKChunkSize, number_non_zeros - base); //count in this chunk. in case number_non_zeros not divisible by topKChunkSize
  ...
  if (m < M) {
    output[batch_idx * int64_t(M) + m] = __float2bfloat16_rn(accumulator);  }
}
...
torch::Tensor sparse_gemv(
    ...
    int64_t keep_count) {// keep_count: exact number of activations retained per vector
  ...
  auto output = torch::empty({batch_count, M}, x.options()); //32 bit output
  ...
  dim3 const grid(static_cast<unsigned>(batch_count),static_cast<unsigned>((M + kThreads - 1) / kThreads)); //kthreads=1 makes roud up
